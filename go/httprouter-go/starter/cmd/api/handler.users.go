...
import (
	...
	"github.com/mrgsrylm/ShaBank/goserver/internal/exceptions"
	"github.com/mrgsrylm/ShaBank/goserver/internal/validator"
)
...
func (app *application) registerUserHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Add the "profiles:read" permission for the new user.
	err = app.models.Permissions.AddForUser(user.ID, "profiles:read")
	if err != nil {
		app.serverErrorResponse(w, r, err)
		return
	}
...
func (app *application) activateUserHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		switch {
		case errors.Is(err, exceptions.ErrEditConflict):
...
// showProfile handles the "GET /v1/users/:id" endpoint and returns a JSON response of the
// requested user record. If there is an error a JSON formatted error is
// returned.
func (app *application) showProfileHandler(w http.ResponseWriter, r *http.Request) {
	...
	profile, err := app.models.Users.GetByID(id)
	if err != nil {
		switch {
		case errors.Is(err, exceptions.ErrRecordNotFound):
			app.notFoundResponse(w, r)
