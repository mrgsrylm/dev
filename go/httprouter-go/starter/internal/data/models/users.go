...
func (m UserModel) Insert(user *User) error {
	query := `
		INSERT INTO users (name, email, password_hash, phone_number, role, activated)
		VALUES ($1, $2, $3, $4, $5, $6)
		RETURNING id, created_at, updated_at, version
		`

	args := []interface{}{user.Name, user.Email, user.Password.hash, user.PhoneNumber, user.Role, user.Activated}
...
func (m UserModel) GetByID(id int64) (*User, error) {
	query := `
		SELECT id, created_at, name, email, password_hash, phone_number, role, activated, version
		FROM users
		WHERE id = $1
		`
...
func (m UserModel) Update(user *User) error {
	query := `
		UPDATE users
		SET name = $1, email = $2, password_hash = $3, phone_number = $4, role = $5, activated = $6, version = version + 1
	...
	if err != nil {
		...
		case err.Error() == `pq: duplicate key value violates unique constraint "users_email_key"`:
			return exceptions.ErrDuplicateEmail
		case errors.Is(err, sql.ErrNoRows):
			return exceptions.ErrEditConflict
		default:
			return err
		}
	}
...
func (m UserModel) GetForToken(tokenScope, tokenPlaintext string) (*User, error) {
	...
	query := `
		SELECT 
			...
			users.password_hash, users.phone_number, users.role, 
